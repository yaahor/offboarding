...
describe(UserApiService.name, () => {
  ...
  describe('getUsers', () => {
    it('should retrieve a list of users', () => {
      const mockUsers: UserDto[] = [
        { id: '1', name: 'User 1', department: 'Department 1', email: 'user1@email', equipments: [], status: 'ACTIVE', },
        { id: '2', name: 'User 2', department: 'Department 1', email: 'user2@email', equipments: [], status: 'OFFBOARDED', },
      ];
  ...
  describe('getUser', () => {
    it('should retrieve a user by id', () => {
      ...
      const req = httpTestingController.expectOne('http://localhost:3000/employees/testID');
      expect(req.request.method).toEqual('GET');
      req.flush(mockUser);
    });
  ...
  describe('conductOffboarding', () => {
    it('should conduct offboarding', () => {
      const mockOnboardingDto: OnboardingDto = { receiver: 'testReceiver' } as OnboardingDto;

      service.conductOffboarding('testID', mockOnboardingDto).subscribe((res) => {
        expect(res).toBeUndefined();
      });

      const req = httpTestingController.expectOne(`http://localhost:3000/users/testID/offboard`);
